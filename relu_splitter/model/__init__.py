...
class WarppedOnnxModel():
    ...
    def get_conv_wb(self, node):
        ...
        try:
            ...
            # self.logger.warning(f"Conv node {node.name} has no bias, assuming node is (Conv v11), treating bias as zero")
        return w,b
    ...
    def save(self, fname: Path, ir_version: int = None):
        if not fname.parent.exists():
            fname.parent.mkdir(parents=True)
        
        if ir_version is not None:
            temp = deepcopy(self._model)
            temp.ir_version = ir_version
            onnx.save(temp, fname)
            return fname
        else:
