    ...
    try:
        while proc.poll() is None:
            ...
            if max_memory >= 0 and mem_usage >= max_memory:
                logger.error(
                    "Out of Memory (terminating process): %s > %s, Duration: %.3fs, MemUsage: %s",
            ...
            if timeout >= 0 and duration_t >= timeout:
                logger.error(
                    "Timeout (terminating process): %.2f > %.2f, Duration: %.3fs, MemUsage: %s", 
                    duration_t, 
                    timeout, 
                    duration_t, 
                    memory_repr(mem_usage)
            ...
            logger.info("Process finished successfully, Duration: %.3fs, MemUsage: %s", duration_t, memory_repr(mem_usage))
            terminate()
